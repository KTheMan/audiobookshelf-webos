<template>
  <div class="fixed top-0 left-0 right-0 layout-wrapper w-full z-50 overflow-hidden pointer-events-none">
    ...
    <div id="side-drawer-panel" class="absolute top-0 right-0 w-64 h-full bg-bg transform transition-transform py-6 pointer-events-auto" :class="show ? '' : 'translate-x-64'" @click.stop>
      <div class="px-6 mb-4">
      ...
      <div class="w-full overflow-y-auto">
        <template v-for="item in navItems">
          ...
          <nuxt-link v-else :to="item.to" :key="item.text" :tabindex="show ? 0 : -1" class="w-full hover:bg-bg/60 flex items-center py-3 px-6 text-fg" :class="currentRoutePath.startsWith(item.to) ? 'bg-bg-hover/50' : 'text-fg-muted'" @click.native="show = false">
            <span class="material-symbols fill text-lg">{{ item.icon }}</span>
            <p class="pl-4">{{ item.text }}</p>
          </nuxt-link>
        </template>
      ...
      <div class="absolute bottom-0 left-0 w-full py-6 px-6 text-fg">
        ...
        <div class="flex items-center">
          ...
          <div class="flex-grow" />
          <div v-if="user" class="flex items-center" :tabindex="show ? 0 : -1" @click="disconnect">
