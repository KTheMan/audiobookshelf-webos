<template>
  <div :key="bookmark.time" :id="`bookmark-row-${bookmark.time}`" class="flex items-center px-1 py-4 justify-start relative" :class="highlight ? 'bg-bg bg-opacity-60' : ' bg-opacity-20'" data-focusable @click="click">
    ...
    <div class="h-full flex items-center justify-end transform w-16 pr-2" @click.stop>
      <span class="material-symbols text-2xl mr-2 text-fg hover:text-yellow-400" data-focusable @click.stop="editClick">edit</span>
      <span class="material-symbols text-2xl text-fg hover:text-error" data-focusable @click.stop="deleteClick">delete</span>
    </div>
  </div>
</template>
